defmodule TimeManager.Schedule do
  @moduledoc """
  ...
  def get_workingtime_by_user!(user_id) do
    query = from w in "workingtimes",
              ...
              select: {w.id, w.start, w.end, w.user}
    Repo.all(query)
  end

  def get_workingtime_by_user_workingID!(user_id, workingtime_id) do
    query = from w in "workingtimes",
              where: w.user == ^user_id and w.id == ^workingtime_id,
              select: {w.start, w.end, w.user}
    Repo.all(query)
  end
